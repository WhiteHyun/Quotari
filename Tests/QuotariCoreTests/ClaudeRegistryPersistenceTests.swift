...
struct ClaudeRegistryRefreshTests {
  ...
  @Test func aNonRotatingCLIGrantIsNotQueuedOnPersistFailure() async throws {
    // The endpoint omitted a rotated refresh token, so the stored token is
    // still reusable and a durable copy could later overwrite a fresher pair.
    let coordinator = ClaudeTokenRefreshCoordinator()
    let store = CapturedAccountStore(
      keychain: InMemoryKeychain().store,
      service: "Test-ClaudeNonRotating-\(UUID().uuidString)"
    )
    ...
    #expect(try strategy.loadDurablePending(source: source) == nil)
  }
